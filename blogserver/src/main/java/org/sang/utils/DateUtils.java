...
public class DateUtils {
	...
	public static Long dateStringToSeconds(String dateString){
		if(dateString.contains("小时")) {
			long t = Long.parseLong(dateString.replace("小时", ""));
			return t * 60 * 60;
		}else if(dateString.contains("天")){
			long t = Long.parseLong(dateString.replace("天", ""));
			return t * 60 * 60 * 24;
		}else if(dateString.contains("个月")){
			long t = Long.parseLong(dateString.replace("个月", ""));
			return t * 60 * 60 * 24 * 30;
		}else if(dateString.contains("年")){
			long t = Long.parseLong(dateString.replace("年", ""));
			return t * 60 * 60 * 24 * 30 * 12;
		}
		throw new IllegalArgumentException("日期字符无法解析！");
	}
}
